...
public class OrderDataMapper {
    ...
    public Order createOrderCommandToOrder(CreateOrderCommand command) {
        ...
        // 1. Chuyển Address DTO -> StreetAddress (Value Object)
        // Lưu ý: Dùng Getter (.getAddress()) vì DTO giờ là Class
        StreetAddress address = new StreetAddress(
                // Không cần UUID.randomUUID() nữa vì StreetAddress là @Embeddable
                UUID.randomUUID(),
        ...
        // 2. Chuyển List<OrderItemDto> -> List<OrderItem> (Domain Entity)
        List<OrderItem> items = command.getItems().stream()
                .map(itemDto -> OrderItem.builder()
                        .productId(new ProductId(itemDto.getProductId()))
                        .quantity(itemDto.getQuantity())
                        .price(new Money(itemDto.getPrice()))
//                        .subTotal(new Money(itemDto.getSubTotal())) // Lấy subTotal từ DTO truyền vào
                        .build())
                .collect(Collectors.toList());
        ...
        return Order.builder()
                ...
                .price(new Money(command.getPrice())) // Lấy tổng tiền từ DTO
                .deliveryAddress(address)
                .items(items)
                .build();
    }
    ...
    public TrackOrderResponse orderToTrackOrderResponse(Order order) {
        return new TrackOrderResponse(
                order.getTrackingId().value(),
                order.getOrderStatus(),
                // Kiểm tra null để tránh lỗi nếu không có failureMessages
                order.getFailureMessages() != null ?
                        Collections.singletonList(order.getFailureMessages()) :
    ...
    /**
     * Chuyển Domain Entity (Order) -> DTO (OrderSummaryResponse)
     * Dùng cho danh sách tất cả đơn hàng
     */
    public OrderSummaryResponse orderToOrderSummaryResponse(Order order) {
        // Format địa chỉ thành 1 chuỗi cho gọn
